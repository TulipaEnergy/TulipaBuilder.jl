@testsnippet NorseFixes begin
    ...
    # They are unfixable because we cannot define `column = nothing` or
    # similar, so the proper solution is to fix the data in TEM
...
@testitem "Comparison of Norse generated via TulipaBuilder" tags = [:integration] setup =
    ...
    for file in readdir(norse_folder, join = true)
        ...
        if table_name in [
            "rep_periods_mapping", # Ignored because it's clustering-specific
            ...
            "assets_timeframe_profiles", # TODO: This should be handled
            "assets_timeframe_partitions", # TODO: This should be handled
            "group_asset", # TODO: This should be handled
        ]
            continue
        end
        DuckDB.query(
